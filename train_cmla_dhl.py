...
def evaluate(s, out, epoch, aux, test_dict, We, vocab, lstm_attention, d, c, mixed=False, logger=None):
    true_list = []
    true_a, true_o = list(), list()
    pred_a, pred_o = list(), list()
...
def __train_with_data(s, lstm_attention, aux, data, L, d, c, len_voc, train_size):
    error = 0.0
    size = 0
...
def __train(dataset_file):
    logger.info('dataset file: {}'.format(dataset_file))
    ...
    lstm_attention = gru_tensor_pdropout_attention.model(
        nh=s['nhidden'], nc=3, ne=100, de=s['emb_dimension'], cs=s['win'],
        csv=1, iteration=1, featdim=0, nt=20, nt_=20)

    # load data
    vocab, train_dict, test_dict = cPickle.load(open(dataset_file, 'rb'))

    logger.info('load word vecs: {}'.format(word_embed_file))
    We = cPickle.load(open(word_embed_file, 'rb'))
    ...
    fout = open('outcomes_gru_tensor_pdropout_res15.txt', 'a')
    ...
    for tdata in [train_dict]:
        ...
        for epoch in range(0, n_epochs):
            ...
            for inst_ind, inst in enumerate(tdata):
                ...
                err, aux, We = __train_with_data(
                    s, lstm_attention, aux, inst, We, word_embed_dim, n_classes, len(vocab), train_size)
                ...
                if inst_ind % 500 == 0 and inst_ind != 0:
                    evaluate(s, fout, epoch, aux, test_dict, We, vocab, lstm_attention, d, c, logger=logger)
